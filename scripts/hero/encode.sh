...
# faststart so the browser reads the index before the whole file arrives, no audio.
ffmpeg -v error -y -framerate 24 -i "$FRAMES/f_%04d.png" -c:v libx264 -preset slow -crf 21 -profile:v high -level 4.0 \
